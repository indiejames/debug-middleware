;;
;;  Modified clojure.trace to mark traces to make them easier to pick up in the REPL output
;;

(ns debug-middleware.trace
  (:use [clojure.pprint]))
...
may be rebound to do anything you like. 'name' is optional."
  ...
  (let [val-str (str value)
        id-str (if id 
                   (str id)
                   "")]
    (println (format "<TRACE: %s>%s</TRACE: %s>" id-str val-str id-str))))
...
(def skip-ns
  "Namespaces that should not be traced"
  '#{clojure.core
     debug-middleware.trace
     debug-middleware.core
     debug-middleware.jdi
     debug-middleware.language-server
...
(defn ^{:skip-wiki true} trace-ns*
  ...
  No-op for clojure.core and debug-middleware namespaces."
  [ns]
  (let [ns (the-ns ns)]
    (when-not (skip-ns (.name ns))
